...
{
    [SerializeField]
    private List<SerializedDictionaryKVPProps<TKey, TValue>> dictionaryList =
        new List<SerializedDictionaryKVPProps<TKey, TValue>>();
    ...
    public new TValue this[TKey key]
    {
        ...
        set
        {
            if (ContainsKey(key))
            {
                base[key] = value; // Update existing entry
            }
            else
            {
                base.Add(key, value); // Add new entry
            }
        }
    }
    ...
    public SerializableDictionary<T, List<TKey>> ReverseKeyValue<TV, T>()
        where TV : List<T>
    {
        var result = new SerializableDictionary<T, List<TKey>>();
